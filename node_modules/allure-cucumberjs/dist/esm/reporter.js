...
import { ALLURE_RUNTIME_MESSAGE_CONTENT_TYPE, ReporterRuntime, applyLinkTemplate, createDefaultWriter, createStepResult, getEnvironmentLabels, getFallbackTestCaseIdLabel, getFrameworkLabel, getHostLabel, getLanguageLabel, getPackageLabel, getProjectName, getThreadLabel, getWorstTestStepResult, md5 } from "allure-js-commons/sdk/reporter";
...
export default class AllureCucumberReporter extends Formatter {
  ...
  onTestCaseStarted(data) {
    ...
    var fullName = "".concat(fullNameBase, "#").concat(pickle.name);
    var testCaseId = md5("".concat(fullNameBase, "#").concat((_scenario$name = scenario === null || scenario === void 0 ? void 0 : scenario.name) !== null && _scenario$name !== void 0 ? _scenario$name : pickle.name));
    var legacyTestCaseId = md5("".concat(posixPath, "#").concat((_scenario$name2 = scenario === null || scenario === void 0 ? void 0 : scenario.name) !== null && _scenario$name2 !== void 0 ? _scenario$name2 : pickle.name));
    var result = {
      name: pickle.name,
    ...
    result.labels.push(getLanguageLabel(), getFrameworkLabel("cucumberjs"), getPackageLabel(getPathRelativeToProjectRoot(pickle)), getFallbackTestCaseIdLabel(legacyTestCaseId), getHostLabel(), getThreadLabel(data.workerId));
    ...
    var titlePath = posixPath.split("/").slice(0, -1).concat([doc === null || doc === void 0 || (_doc$feature4 = doc.feature) === null || _doc$feature4 === void 0 ? void 0 : _doc$feature4.name].filter(Boolean));
  ...
  onAttachment(message) {
    ...
    var fixtureUuid = message.testCaseStartedId ? this.fixtureUuids.get(message.testCaseStartedId) : undefined;
    var testUuid = message.testCaseStartedId ? this.testResultUuids.get(message.testCaseStartedId) : undefined;
    ...
    if (message.mediaType === ALLURE_RUNTIME_MESSAGE_CONTENT_TYPE) {
      if (fixtureUuid && testUuid) {
        var runtimeMessages = this.parseRuntimeMessages(message.body);
        var hasPlaywrightTrace = runtimeMessages.some(msg => (msg.type === "attachment_content" || msg.type === "attachment_path") && msg.data.contentType === ContentType.PLAYWRIGHT_TRACE);
  ...
  parseRuntimeMessages(messageBody) {
    var parsedMessage = JSON.parse(messageBody);
    return Array.isArray(parsedMessage) ? parsedMessage : [parsedMessage];
  }
  applyRuntimeAttachmentMessages(rootUuid, messageBody) {
    var runtimeMessages = this.parseRuntimeMessages(messageBody);
    if (rootUuid) {
